...
def ema_trend_strategy(candles):
    ...
    # Берем EMA от всей доступной истории на текущий шаг
    e9 = ema(closes, 9)
    e21 = ema(closes, 21)
...
def breakout_strategy(candles):
    ...
    # Предыдущие 20 свечей для расчета хай/лоу
    prev = candles[-21:-1]
    last = candles[-1]
...
def rsi_strategy(candles):
    ...
    # Берем последние 15 свечей из переданного окна
...
def macd_strategy(candles):
    ...
    # Берем предыдущие значения для фиксации пересечения
    prev_closes = closes[:-1]
    prev_fast = ema(prev_closes, 12)
    prev_slow = ema(prev_closes, 26)
